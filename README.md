...
this closed V1 shape (missing required or additional properties are rejected):

```ts
{
  tenantId: string,        // 1–128 characters; stable for the deployment
  schemaVersion: 1,
  // Optional one-time proof generated by a trusted deployment pipeline when explicitly adopting
  // existing admin state. Do not construct this value manually.
  adoptExistingConfigDigest?: string, // lowercase 64-character SHA-256
  config: {
...
Genuinely unmarked, non-default admin configuration fails closed unless a trusted deployment
pipeline explicitly provides `adoptExistingConfigDigest`. The proof is SHA-256 over a canonical
purpose tag, the tenant ID, and the exact serialized existing configuration. Adoption succeeds only
when the raw authoritative Durable Object value and the KV mirror are identical and match that
proof. It records a completed marker without rewriting either configuration copy, including for
legacy objects that predate newer defaulted fields or whose existing value equals the defaults. The
proof is excluded from the canonical V1 bootstrap digest, so it can be omitted after adoption.
Missing, stale, cross-tenant, or inconsistent proofs fail closed. An interrupted `pending`
initialization resumes only when its tenant, version, digest, and authoritative configuration all
match.

The deployment pipeline and the Cloudflare account's Worker/KV write authorities are trusted. This
proof prevents accidental cross-tenant adoption and state mismatch; it is not a secret or a defense
against an actor who can replace Worker code, deployment bindings, or KV after adoption.
