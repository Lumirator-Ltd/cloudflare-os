...
### Sharing and sensitive-observation concurrency

The Overseer serializes sharing transitions against observations marked `prohibitAllSharing`. Access-granting mutations (`addCollaborator`, share-link creation/key minting/redemption, and future operations with the same effect) register an in-memory transition before their first await and clear it in `finally`. A sensitive observation registers its sharing-lockdown transition before checking the graph. Whichever transition starts first may finish; the other rejects without granting access or authorizing the observation. Sharing mutations and revocations also reject if lockdown is in progress or `prohibitAllSharing` is already set. Action application is mutually excluded with both lockdown and effective sharing revocation, preventing an abort from interrupting an external mutation and leaving it replayable.

Revocation is fail-closed in the other direction. `removeCollaborator` and `revokeShareLink` register a revocation transition before changing graph reachability, so a sensitive observation cannot overlap the interval between edge removal and live-session invalidation. The complete permission-graph mutation, including re-rooting and affected-user computation, runs inside one synchronous Durable Object storage transaction; an exception after any intermediate write rolls the complete mutation back before the transition is released. If no user's access or role changed, the transition ends normally. Otherwise it remains active through observer/listing cleanup and the scheduled DO abort; the restart discards the in-memory transition together with the stale sessions. Revocation reports success only after the graph is durable and the delayed abort is registered with `ctx.waitUntil`. Unexpected cleanup failure still leaves the transition active and schedules the restart. A durability failure is reported to the caller and registers a fail-closed abort, so account-only observations cannot proceed in the damaged instance.

These transitions are deliberately in memory: they coordinate requests within one live Durable Object instance, while the durable graph and `prohibitAllSharing` flag remain authoritative across restarts.
